...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            case .permission:
                VStack(spacing: 12) {
                    Text("Locally needs the microphone to hear speech. Audio is processed entirely on-device.")
                        .font(.footnote)
                        .multilineTextAlignment(.center)
                        .foregroundStyle(.secondary)
                        .padding(.horizontal, 32)
                    if micDenied {
                        Text("Microphone access is off. Locally cannot transcribe without it.")
                            .font(.footnote)
                            .foregroundStyle(.red)
