...
struct TranscriptSegmenter: Sendable {
    ...
    func process(_ event: TranscriptionEvent, language: AppLanguage) -> Output? {
        switch event {
        case .volatile(let text):
            let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !trimmed.isEmpty else { return nil }
            return Output(kind: .volatileUpdate, text: trimmed)

        case .finalized(let text):
            let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !trimmed.isEmpty else {
                return Output(kind: .discard, text: "")
            }
            let refine = isWorthRefining(trimmed, language: language)
            return Output(kind: .finalized(refine: refine), text: trimmed)
